...
$$
**变异系数**（coefficient  of variation，CV）：标准差与期望的比值。变异系数是个无量纲的量，从而消除了量纲对波动的影响。
$$
\text{CV}=\sigma/\mu
$$
...
$$
其中，$d_i$ 表示数据点 $(x_i,y_i)$ 的秩的差值 $d_i=\text{rank}(x_i)-\text{rank}(y_i)$ 

(3) Kendall秩相关系数：是基于样本数据对之间的关系来进行相关系数的强弱的分析，数据对可以分为同序对（Concordant pairs）和异序对（Discordant pairs）
$$
...
**核密度估计**（kernel density estimate，kde）：本质是直方图的拟合曲线，其可以看作是概率密度图。
