...
tags:
  - C++
cover: /img/c-programming.png
top_img: /img/c-program-icon.png
abbrlink: 3f4b6fbd
...
函数在调用之前必须先声明。函数声明告诉编译器参数的数量、参数的数据类型以及返回函数的类型。当您在一个源文件中定义函数且在另一个文件中调用函数时，函数声明是必需的。在这种情况下，您应该在调用函数的文件顶部声明函数。
...
一个项目往往包含多个源码文件，通常的做法是创建一个以 `.h` 作为扩展名的 **头文件**，放置所有在`.cpp`里面定义的函数声明等内容。使用预处理器指令 `#include` 用于编译时将其他源码文件，加载进入当前文件。
...
例如，我们想在主程序中引入一个函数 `calculate_age.cpp`
...
需要创建一个同名的头文件 `calculate_age.h` ，里边只放入函数声明
...
int main(void) {
  std::cout << calculateAge(1983) << std::endl;
}
```
...
```sh
g++ -o main main.cpp calculate_age.cpp
```


## 条件编译

头文件里面还可以加载其他头文件，因此有可能产生重复加载，这将产生错误。为了防止这种情况，标准的做法是把文件的整个内容放在条件编译语句中。条件编译指令包括：`#if, #ifdef, #ifndef, else, #elif, #endif`
...
这种结构就是通常所说的包装器 **#ifndef**。当再次引用 HEADER_FILE 时，条件为假。此时，预处理器会跳过文件的整个内容，编译器会忽略它。
